...
mapviewFarm() +
  mapview(soil_data[soil_data$camada_id == 1, "observacao_id"], layer.name = "0-20 cm") +
  mapview(soil_data[soil_data$camada_id == 2, "observacao_id"], layer.name = "20-40 cm")
```

Vamos verificar a distribuição empírica de uma das propriedades do solo a serem mapeadas, por exemplo, a 
saturação por bases, utilizada no cálculo da necessidade de calagem. Para isso, usamos a função
`pedometrics::plotHD`.

```{r, fig.asp=1}
plotHD(soil_data[["bases_saturacao_calc"]])
```
...
Uma análise exploratória espacial dos dados pode ser feita com o auxílio da função `pedometrics::plotESDA`. 
Vamos usar apenas os dados da camada de 0-20 cm de profundidade, que é a camada de interesse nesse estudo.

```{r, fig.asp=1}
tmp <- 
  soil_data %>% 
  dplyr::filter(camada_id == 1)
plotESDA(z = tmp[["bases_saturacao_calc"]], lat = tmp[["y"]], lon = tmp[["x"]])
```

## Calibração dos modelos

```{r}
f <- bases_saturacao_calc ~ x + y + slope + plan_curvature + profile_curvature + general_curvature + twi + catchment_area + catchment_slope + elevation + CE37 + CE75 + ndvi_2013_04_22 + ndvi_2016_01_25 + ndvi_2018_04_20 + ndvi_variation + fields + water_tank_dist + camada_id
```
...
propriedade do solo, a saturação por bases, enquanto as variáveis preditoras são as covariáveis ambientais. 
Além dos atributos de terreno, das informações sobre o uso da terra e as práticas de manejo e conservação do 
solo, das imagens de satélite e índices de vegetação, e das informações indiretas sobre as propriedades do solo,
usaremos como covariáveis ambas as coordenadas espaciais e o código de identificação da camada amostrada. A
última é uma covariável importante pois, como sabemos, os dados provém das camadas de 0-10 e 20-40 cm de
profundidade. Em geral, a camada superficial apresenta níveis superiores de nutrientes, o que poderá ser
explicado pelo modelo usando a covariável `camada_id`. Ao total, são 18 covariáiveis, sendo que a covariável 
dos piquetes de pastoreio se desdobra em tantas covariáveis quantos foram os seus níveis, menos 1.
...
fit_idw <- 
  ...
  dplyr::filter(camada_id == 1) %>% 
  gstat(
  id = "bases_saturacao_calc", formula = bases_saturacao_calc ~ 1, locations = ~ x + y, data = ., nmax = 7, 
  set = list(idp = .5)) %T>% 
  print()
cv_idw <-
  soil_data %>% 
  as.data.frame() %>% 
  dplyr::filter(camada_id == 1) %>% 
  gstat.cv(fit_idw, formula = bases_saturacao_calc ~ 1, locations = ~ x + y, data = ., nmax = 7, 
  set = list(idp = .5), verbose = FALSE, debug.level = 0)
```
...
```{r}
soil_data %>% 
  mutate(glm = fit_glm$pred$pred - bases_saturacao_calc,
         rf  = fit_rf$pred$pred - bases_saturacao_calc) %>% 
  dplyr::filter(camada_id == 1) %>% 
  mutate(idw = cv_idw$bases_saturacao_calc.pred - bases_saturacao_calc) %>% 
  as.data.frame() %>% 
  dplyr::select(glm, rf, idw) %>% 
  stack() %>% 
  mutate(residual = rep(mean(cv_idw$observed) - cv_idw$observed, 3)) %>% 
  dplyr::group_by(ind) %>%
  dplyr::summarise(
    ME = mean(values),
    MedE = median(values),
...
pred_idw <- interpolate(covar_data, fit_idw)
```

```{r, fig.asp=1, fig.width=12}
par(mfrow = c(2, 2))
plot(pred_glm, main = "glm")
plot(pred_rf, main = "rf")
plot(pred_idw, main = "idw")
```
...
mapview(pred_rf) +
mapview(pred_idw) +
  soil_data %>% dplyr::filter(camada_id == 1) %>% dplyr::select(bases_saturacao_calc) %>% mapview() +
  mapview(contour_lines)
```
...
tmp <- 
  soil_data %>% 
  mutate(
    res_rf  = fit_rf$finalModel$predictions - bases_saturacao_calc,
    res_glm = fit_glm$finalModel$fitted.values - bases_saturacao_calc) %>% 
  dplyr::filter(camada_id == 1)
