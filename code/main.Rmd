...
```{r}
study_area <- sf::read_sf("../data/vector/contornoFazenda.shp")
study_area$id <- 1
mapview::mapview(study_area, map.types = "Esri.WorldImagery")
```
...
study_area %>% 
  ...
  mapview::mapview(soil_data["observacao_id"], layer.name = "samples")
```
...
```{r}
fields <- 
  sf::read_sf("../data/vector/PIQUETES_CAIUA.shp") %>% 
...
```{r}
f <- log1p(fosforo_resina) ~ slope + general_curvature + plan_curvature + profile_curvature + twi + catch_area + catch_slope + ndvi_2013_04_22 + ndvi_2016_01_25 + ndvi_2018_04_20 + ec + dem_30m + camada_id
fit_rf <- 
  soil_data %>% 
  caret::train(
    f, method = "ranger", data = ., na.action = na.omit, trControl = caret::trainControl(method = "LOOCV"), 
    tuneGrid = data.frame(mtry = 4, splitrule = "variance", min.node.size = 5)) %T>% 
  print()
```
...
predictions <- raster::predict(covar_data, fit_rf)
mapviewRaster(predictions) + 
  mapview::mapview(soil_data[soil_data$camada_id == 1, "fosforo_resina_log"], legend = FALSE)
```
